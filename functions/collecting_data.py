...
class CollectingData(SavePictureFile, Utils):

    def __init__(self, browser: Selenium, search_phrase: str):
        self.browser = browser
        self.search_phrase = search_phrase

    def get_only_image_name(self, img_url: str) -> str:
        """
    ...
    def get_img_from_element(self, single_new: WebElement) -> str:
        """
        ...
        try:
            ...
            img = self.browser.get_element_attribute(img_tag, "src")
        except NoSuchElementException:
            logging.error("Can't find element")
        return img


    def get_description_from_element(self, single_new: WebElement) -> str:
        """
        ...
        try:
            ...
            description = self.browser.get_text(description_tag)
        except NoSuchElementException:
            logging.error("Can't find element")
        return description

    def get_list_result(self) -> list:
        """
        ...
        #Process
        news_count = self.browser.get_element_count(li_news_path)
        news_list_element = self.browser.get_webelements(li_news_path)
        ...
        for new_li in range(news_count):
            ...
            title = self.browser.get_text(title_tag)
            date = self.browser.get_text(date_tag)
            description = self.get_description_from_element(li)
            img_url = self.get_img_from_element(li)
            img = self.get_only_image_name(img_url)
            whole_text = title + ", " + description
            count_search_phrase = self.search_term_count_by_new(whole_text, self.search_phrase)
            amount_money = self.find_money_in_text(whole_text)
            self.save_picture(img_url, img)
